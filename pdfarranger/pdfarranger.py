...
class PdfArranger(Gtk.Application):
    ...
    def close_application(self, _widget=None, _event=None, _data=None):
        ...
        self.pdfqueue = []
        gc.collect()
        self.config.window_size = self.window.get_size()
        self.config.maximized = self.window.is_maximized()
        self.config.zoom_level = self.zoom_level
        self.config.position = self.window.get_position()
        self.config.save()
    ...
    def save(self, mode, file_out):
        ...
        if self.config.content_loss_warning:
            res, self.config.content_loss_warning = exporter.check_content(self.window, self.pdfqueue)
